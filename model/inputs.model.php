...
class inputsModel extends Model {
    ...
    */
    ...
    function get_input_textarea($id, $val, $class='', $lbl='', $placeholder='', $err_desc='', $min_length=false, $max_length=false, $allow_empty=false, $rows=3, $classdiv='form-group', $arr='') {
        $val = $this->safe_show($val);
        ...
        $o  = '<div class="'.$classdiv.'">';
        if (strlen($lbl) > 0) $o .= '<label>'.$lbl.'</label>';
        $o .=   '<textarea '.$aux_required.' id="'.$id.'" name="'.$id.$arr.'" class="'.$class.'" placeholder="'.$placeholder.'" '.$aux_min_length.' '.$aux_max_lenght.' title="'.$err_desc.'" rows="'.$rows.'">';
        $o .=       $val;
        $o .=   '</textarea>';
        $o .= '</div>';
    ...
    function get_input_number($id, $val, $class='', $lbl='', $placeholder='', $err_desc='', $min=false, $max=false, $step='int', $allow_empty=false, $classdiv='form-group', $arr='') {
        $val = $this->safe_show($val);
        ...
        $o  = '<div class="'.$classdiv.'">';
        if (strlen($lbl) > 0) $o .= '<label>'.$lbl.'</label>';
        $o .=   '<input type="number" '.$aux_required.' id="'.$id.'" name="'.$id.$arr.'" value="'.$val.'" class="'.$class.'" placeholder="'.$placeholder.'" '.
    ...
    function get_input_radio($id, $val, $arr_opt, $class='', $lbl='', $allow_empty=false, $sep=false, $classdiv='form-group', $arr='') {
        $val = $this->safe_show($val);
        ...
        foreach ($arr_opt as $k => $v) {
            ...
            $o .=           '<input type="radio" name="'.$id.$arr.'" id="'.$id.$k.'" value="'.$k.'" autocomplete="off" '.$aux_checked.' '.$aux_required.'> '.$v;
            $o .=       '</label>';
        }
    ...
    */
    function get_input_img($id, $val, $ruta_archivos, $class='', $lbl='', $required='required', $multiple=false, $maxFileCount=false, $arr='[]', $id_lang=false, $id_articulo=false) {

        $ruta_inicio = $ruta_archivos;
        $ruta_archivos = $ruta_archivos.'img/productos/';
        $aux_multiple = '';
        ...
        if (count($val) > 0) {
            ...
            foreach($val as $value){
                if($value!=""){
                    ...
                    //$preview_config .= '{caption: "'.$value.'", filename: "'.$value.'", url: "'.$ruta_inicio.'del.php?id_lang='.$id_lang_mod.'"&id_articulo='.$id_articulo_mod.'}';
                    $preview_config .= '{caption: "'.$value.'", filename: "'.$value.'", url: "'.$ruta_inicio.'del.php?id_lang='.$id_lang.'&id_articulo='.$id_articulo.'&del='.$value.'"}';
        ...
        $o .=       '<input '.$aux_imagen_categoria_required.' id="'.$id.'" name="'.$id.$arr.'" type="file" '.$aux_multiple.'>';
        $o .=   '</div>';
        $o .= '</div>';
    ...
    function get_select($id, $val, $arr_opt, $class=false, $lbl=false, $onChange=false, $multiple=false, $classdiv='form-group', $arr='') {
        ...
        $o .=   '<select '.$aux_multiple.' id="'.$id.'" name="'.$id.$arr.'" class="'.$class.'" '.$aux_onChange.'>';
